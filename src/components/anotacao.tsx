...
export function Anotacao({ numero, titulo, descricao, isIncluido }: { numero: number, titulo: string, descricao: string, isIncluido: boolean }) {
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await fetch('/saveAnotacao', {
        ...
        body: JSON.stringify({ titulo: tituloAnotacao, descricao: descricaoAnotacao, id: numero })
      });
  ...
  return (
    <div className="flex flex-col items-center justify-center border-2 border-gray-300 rounded-lg p-4 h-full">
      ...
      />
      <textarea 
        onChange={(e) => setDescricaoAnotacao(e.target.value)} 
        value={descricaoAnotacao} 
